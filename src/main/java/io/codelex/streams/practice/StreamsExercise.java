...
public class StreamsExercise {
    ...
    public static double getAverageAge(List<User> users) {
        return users.stream().mapToDouble(User::getAge).average().orElseThrow(IllegalStateException::new);

    }

    public static Integer getMaxAge(List<User> users) {
        return users.stream().mapToInt(User::getAge).max().orElseThrow(IllegalStateException::new);
    }

    public static Integer getMinAge(List<User> users) {
        return users.stream().mapToInt(User::getAge).min().orElseThrow(IllegalStateException::new);
    }
    ...
    public static List<Integer> generate10RandomNumbers() {
        return new Random().ints(10, 1, 100).boxed().collect(Collectors.toList());
    }
